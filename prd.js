      ...
      blocks: [
        {
          ...
          items: [
            ...
            { tag: "done", text: { ko: "AI가 하는 부분(구성안 제안·초안 작성·심사기준 평가·검색 키워드 추천·공모요령 PDF에서 심사기준 추출)은 무료 모델(Groq)로 실제 연결해 5개 전부 확인했다. 연결 과정에서 모델 단종과 빈 응답 버그를 발견해 고쳤다.", ja: "AIが担う部分（構成案の提案・下書き作成・審査基準の評価・検索キーワードの提案・公募要領PDFからの審査基準抽出）は無料モデル（Groq）で実際に接続し、5つ全部を確認した。接続の過程でモデルの廃止と空応答のバグを発見して修正した。", en: "The AI parts (outline suggestions, drafting, criteria evaluation, keyword recommendations, criteria extraction from call PDFs) are now connected to a free model (Groq) and all five were checked with real calls. Along the way I found and fixed a discontinued model and an empty-response bug." } },
      ...
      blocks: [
        {
          type: "list",
          items: [
            { tag: "done", text: { ko: "완료: 7개 화면, 로그인·기기 간 동기화, 고객 관점 UAT와 수정, AI 생성 기능을 무료 모델(Groq)로 연결·검증, 비밀번호 변경", ja: "完了：7つの画面、ログイン・端末間同期、顧客視点のUATと修正、AI生成機能を無料モデル（Groq）に接続・検証、パスワード変更", en: "Done: seven screens, login and cross-device sync, customer-side UAT and fixes, AI generation connected to a free model (Groq) and verified, password change" } },
            { tag: "plan", text: { ko: "지금: 중소기업 3~5곳 베타 모집·관찰, 성공 지표 계측 추가", ja: "今：中小企業3〜5社のベータ募集・観察、成功指標の計測を追加", en: "Now: recruit and observe 3–5 SMEs in beta, add tracking for the success metrics" } },
            { tag: "plan", text: { ko: "다음: 다른 보조금 공모요령까지 심사기준 자동 추출 범위 확대", ja: "次：他の補助金の公募要領まで審査基準の自動抽出範囲を拡大", en: "Next: widen automatic criteria extraction to other subsidies' calls" } },
            { tag: "plan", text: { ko: "그다음: 이메일로 비밀번호 재설정 (메일 발송 서비스 연결 필요)", ja: "その次：メールによるパスワード再設定（メール送信サービスの接続が必要）", en: "Later: email-based password reset (needs a transactional-email service)" } }
          ]
        }
      ]
    },
      ...
      blocks: [
        {
          ...
          items: [
            ...
            { tag: "done", text: { ko: "무료 모델의 일본어 비즈니스 문장 품질: 5개 기능을 실제 호출로 확인함 (사실 왜곡 없이 준 답변만 사용, 빠진 정보는 지어내지 않고 '없다'고 표시). 다만 표본이 적어 대량·장기 검증은 아직", ja: "無料モデルの日本語ビジネス文章の品質：5つの機能を実際の呼び出しで確認（事実を歪めず回答のみを使用、不足情報は作らず「ない」と表示）。ただしサンプルが少なく、大量・長期の検証はまだ", en: "Free model's Japanese business-writing quality: checked all five features with real calls (used only the given facts, no fabrication, flagged missing info as missing instead of inventing it). Sample size is small, though — no large-scale or long-term check yet" } },
            { tag: "none", text: { ko: "무료 모델 라인업이 예고 없이 바뀜: 연결 첫날 기존에 쓰던 모델이 단종된 걸 발견함. 무료 티어라 언제든 다시 바뀔 수 있어 정기적인 확인이 필요", ja: "無料モデルのラインナップが予告なく変わる：接続初日に、使っていたモデルが廃止されていたことが判明。無料枠のためいつまた変わってもおかしくなく、定期的な確認が必要", en: "The free model lineup changes without notice: on day one of connecting, the model already coded in turned out to be discontinued. Being a free tier, it can change again anytime, so it needs periodic checking" } },
